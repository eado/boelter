...
async function main() {
  ...
  for (let player in players) {
    players[player].socket.send(
      "ROLE: " + (players[player].impostor ? "impostor" : "player"),
    );
  }
    ...
    for (let player in players) {
      ...
      if (players[player].impostor) {
        if (!roomLens[idx]) {
          ...
          players[player].question =
            roomQs[idx][Math.floor(Math.random() * roomLens[idx])];
        }
      } else players[player].question = roomQs[idx][--roomLens[idx]];
      content += `${player}: At: ${players[player].room}, Q: ${players[player].question}\n`;
      players[player].socket.send(
        `ROOM: ${players[player].room}, QUESTION: ${players[player].question}`,
      );
    }
